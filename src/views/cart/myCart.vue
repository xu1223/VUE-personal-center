        ...
        <div class="bag-right">
            ...
            <div class="bag-coupon">
                ...
                <v-form-item label :label-col="labelCol" :wrapper-col="wrapperCol" :help="coupon_help" 
                    ...
                    <a @click.prevent="codeApply(coupon_code)">APPLY</a>
                </v-form-item>
                </v-form>
            </div>
        </div>
  ...
  methods: {
    ...
    drawerConfirmBtn(forms){
      ...
      if(this.idrawerType==1){
        ...
        if(arr2!=''){
          this.$Http.cart.wishDo({ cart_ids: arr2}).then(res => {
        ...
        if(arr3!=''){
          this.$Http.cart.cartDel({ cart_ids: arr3}).then(res => {
              let { code, msg, data } = res;
              if (code == 200) {
                  this.idrawer=false;
        ...
        //批量删除购物车
        this.$Http.cart.cartDel({ cart_ids: arr1}).then(res => {
            let { code, msg, data } = res;
            if (code == 200) {
                this.idrawer=false;
        ...
        let arr2=[];
        arr2.push(this.item.id)
        this.$Http.cart.cartDel({ cart_ids: arr2 }).then(res => {
            let { code, msg, data } = res;
            if (code == 200) {
              this.idrawer=false;
              const bag_vux = {
    ...
    codeApply(coupon_code){
        ...
        if (coupon_code == "") {
          ...
          this.$Http.cart.checkCouponAvailabilityByCart({ cart_ids:this.cart_ids,coupon_code:coupon_code}).then(res => {
    ...
    getCarts(){
        this.$Http.cart.getCarts().then(res => {
            let { code, msg, data } = res;
            if (code == 200) {
    ...
    getFeaturedProducts(){
      this.$Http.cart.getFeaturedProducts().then(res => {
          let { code, msg, data } = res;
          if (code == 200) {
            if(data['is_featured']){
    ...
    getRecentlyViewed(){
      this.$Http.cart.getRecentlyViewed().then(res => {
          let { code, msg, data } = res;
          if (code == 200) {
            if(data.total>0){
