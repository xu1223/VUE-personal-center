    ...
    <div class="payment-banner">
        <div class="payment-left">
            <div class="payment-item">
                ...
                <v-checkbox v-if="!signType" style="margin-left:10px;" v-model="is_register" :true-value="1" :false-value="0" class="checkout">Register at the same time</v-checkbox>
            </div>
...
export default {
  ...
  methods: {
    logOut(){
        this.$store.commit("bag/wish_total_num", -1);
        this.$store.commit("bag/bag_total_num", -1);
    ...
    syncOrderTradingInfo() {
      this.$Http.payment.syncOrderTradingInfo({...this.sync}).then(res => {
        let { data, code , msg } = res;
        if (code == 200) {
        }
      });
    ...
    getOrderPaymentDetail () {
      this.$Http.payment.getOrderPaymentDetail({ invoice_no: this.$route.query.invoice_no }).then(res => {
        let { code, data, msg } = res;
        if (code == 200) {
            this.data=data;
